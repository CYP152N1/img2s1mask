# バイナリデータボックスマスキングプログラム README

## 概要

このプログラムは、バイナリ画像ファイルに対して、特定の矩形領域（ボックス）をマスクまたは抽出する機能を提供します。指定されたボックス内のピクセルをマスクしたり、ボックス以外の領域をすべてマスクすることが可能です。また、ボックス内の個々のピクセル値を変更することもできます。
